...
- 在Web开发中，最长遇到的问题就是处理数据结构，在后端生成一个便于实现需求功能的数据结构，生成的数据一般从数据库中获取，过获取的数据进行多层for循环外加if判断是经常要遇到的情景，但是这样效率较低。
- 字典通过索引取值的速度非常快，我们可以实现将一种映射关系存放到字典中，在for循环中使用字典索引来代替嵌套的for循环
	- 在下面例子中
		1. 为了增加例子的可读性，将用户session验证环节使用装饰器实现，当需要验证的页面比较多的时候，我们应该使用中间件而不是装饰器
		2. 密码使用md5加密，当然也可以在加密的时候对密码加盐，防止撞库破解密码
