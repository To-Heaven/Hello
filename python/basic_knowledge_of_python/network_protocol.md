...
- 软件由操作硬件的需求，但是不能直接操纵硬件，需要通过操作系统来完成
- 硬件C/S架构
	- 打印机
- 软件C/S架构
	- qq，浏览器等
...
- socket首发信息的原理
	...
	- 服务器主机接收到该数据包之后，将该数据包存放在主机内存缓存中，根据recv指定长度进行接收缓存池中的数据
		- 上述过程中牵扯到了cpu内核态和用户态之间的相互切换，当应用程序发出调用网卡的请求时，cpu的运行状态就从拥用户态转换成内核态
...
- 与tcp的区别
	...
	2. tcp再客户端和服务端之间建立了双向链接，而udp不会再两者之间建立连接
