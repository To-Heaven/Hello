...
## 操作系统、应用程序与文件
- 数据最先产生更与内存，而应用程序在内存中产生的数据保存到硬盘中才能永久保存，但应用程序想要操作硬盘必须通过操作系统。操作系统为应用程序提供了处理文件的接口，并且提供了文件这个抽象的对象。应用程序通过操作系统操作文件就能实现对硬盘中数据的操作。
...
#### 二进制模式与文本模式
对于文本文件
- 文本文件在磁盘中是以**编码之后的二进制保存的**，因此我们使用文本模式打开的时候需要指定保存文件时使用的编码方式，这样再将文件从磁盘读取到内存的时候就可以将文件中的内容正确的解码到内存中。

对于二进制文件
- 由于二进制文件在保存进磁盘时就是以直接以二进制保存的，中间不存在编码的过程，因此我们在将磁盘中的二进制数据读取到内存的时候不需要指定编码方式。
...
- f.read(size=-1)
	...
	- size: size为正数的时候，返回size个的数据，为负数的时候调用readall()
		...
		- 对于二进制文件：size单位为字节
- f.readline(limit=-1)
	- 读取文件的一行数据，当读取完第一行之后，光标会移动到第二行行首
	- 如果limit>0，返回指定获取这一行中前limit个**字符**
	- 注意
		- 如果文件中某一行为空内容，实际上会读取到一个`\r\n`换行符
		- 当读取到最后一行之后，再往下读取就会返回一个`None`，可以通过它来判断，文件是否读取结束

```python
file_obj = open(file='file_operation_instance', mode='r', encoding='utf-8')
...
- f.readlines(line_num = -1)
	- 返回一个存放每一行内容构成的列表
	- line_num: 大于零时，表示列表中所有元素的最大字符数，如果该字符数在某一行行首和行尾字符数范围内，就会读取该一整行。然而这个参数并没有什么用途。

```python
file_obj = open(file='file_operation_instance', mode='r', encoding='utf-8')

print(file_obj.readlines(8))		# ['asdasd\n', 'asdsa\n']
```
